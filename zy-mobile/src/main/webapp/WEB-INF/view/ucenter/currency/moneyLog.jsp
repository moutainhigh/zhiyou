				...
				if(page.data.length) {
					...
					for(var i in pageData){
						...
						var html = '<div class="list-item">'
							+ '<div class="title lh-24 text-ellipsis">' + item.title + '</div>'
							+ '<div class="inout ' + (item.inOut == '收入' ? 'currency-in' : 'currency-out') + ' lh-24 text-right">' + item.transAmount.toFixed(2) + '</div>'
							+ '<div class="clearfix lh-24">'
							+ '<span class="left fs-12 font-999">' + item.transTimeLabel + '</span>'
							+ '<span class="right fs-12 font-999">余额: ' + item.afterAmount.toFixed(2) + '</span>'
							+ '</div></div>';
