...
import com.zy.common.util.Reflections;
...
public class SecureModelAttributeMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public final boolean supportsParameter(MethodParameter parameter) {
		return parameter.hasParameterAnnotation(SecureModelAttribute.class);
	}
	...
	public final Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		String[] clearFields = annot.clear();
		for (String clearField : clearFields) {
			if (StringUtils.hasText(clearField)) {
				Object clearFiledValue = Reflections.invokeGetter(attribute, clearField);
				if(clearFiledValue != null) {
					Class<?> clearFiledClass = clearFiledValue.getClass();
					if(Map.class.isAssignableFrom(clearFiledClass) || Collection.class.isAssignableFrom(clearFiledClass)) {
						Reflections.invokeMethod(clearFiledValue, "clear", null, null);
					} else {
						Reflections.setFieldValue(attribute, clearField, null);
					}
				}
			}
		}
		...
		if (binder.getTarget() != null) {
			...
			// 设置绑定允许的字段和禁止的字段
			if (annot != null) {
				String[] allowedFields = annot.allowed();
				String[] disallowedFields = annot.disallowed();
