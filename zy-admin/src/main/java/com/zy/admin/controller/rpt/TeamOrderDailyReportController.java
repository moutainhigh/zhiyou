...
@Controller
@RequestMapping("/report/teamOrderDaily")
public class TeamOrderDailyReportController {

	@Autowired
	private LocalCacheComponent localCacheComponent;

	@RequiresPermissions("teamOrderDaily:view")
	@RequestMapping(method = RequestMethod.GET)
	public String list() {
		return "rpt/teamOrderDailyReport";
	}
	...
	public Grid<TeamMonthReportVo> ajax() {
		...
		if (dayOfMonth == 1) {  //今天是月初 查看的是上个月1号-月底的 以及上上个月1号-月底
			localDate = localDate.minusDays(1);
			lastLocalDate = localDate.minusMonths(2);
		} else {
			lastLocalDate = localDate.minusMonths(1);
		}

		Date currentMonthBegin = getBeginDateTime(localDate);
		Date currentMonthEnd= getEndDateTime(localDate);
		Date lastMonthBegin = getBeginDateTime(lastLocalDate);
		Date lastMonthEnd = getEndDateTime(lastLocalDate);
		...
		List<Order> filterOrders = orders.stream()
				...
				.filter(v -> v.getPaidTime().before(currentMonthEnd) && v.getPaidTime().after(lastMonthBegin))
	...
	private Date getBeginDateTime(LocalDate localDate) {
		LocalDate newLocalDate = LocalDate.of(localDate.getYear(), localDate.getMonthValue(), 1);
		LocalDateTime localDateTime = LocalDateTime.of(newLocalDate, LocalTime.parse("00:00:00"));
		Instant instant = localDateTime.atZone(ZoneId.systemDefault()).toInstant();
		return Date.from(instant);
	}

	private Date getEndDateTime(LocalDate localDate) {
		LocalDate newLocalDate = LocalDate.of(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
		LocalDateTime localDateTime = LocalDateTime.of(newLocalDate, LocalTime.parse("23:59:59"));
		Instant instant = localDateTime.atZone(ZoneId.systemDefault()).toInstant();
		return Date.from(instant);
	}
	...
	public static void main(String[] args) {
		String name = "husky";
		String nickname = "HUSKY";
		System.out.printf("%s and %S 你好：", name, nickname);
	}
}
