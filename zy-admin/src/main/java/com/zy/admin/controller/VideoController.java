...
public class VideoController {
	...
	final Logger logger = LoggerFactory.getLogger(VideoController.class);
	...
	public Result<?> upload(MultipartFile file, HttpServletRequest request, Model model, Principal principal) {
		try {
			GcUtils.checkVideo(file);
			String videoUrl = aliyunOssSupport.putPublicObject(Constants.ALIYUN_BUCKET_NAME_IMAGE, "image/", file, Constants.ALIYUN_URL_IMAGE);// 上传阿里云OSS
			return new ResultBuilder<>().data(videoUrl).build();
		} catch (Exception e) {
