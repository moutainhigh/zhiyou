...
public class AudioController {
	...
	final Logger logger = LoggerFactory.getLogger(AudioController.class);
	...
	public Result<?> upload(MultipartFile file, HttpServletRequest request, Model model, Principal principal) {
		try {
			GcUtils.checkAudio(file);
			String audioUrl = aliyunOssSupport.putPublicObject(Constants.ALIYUN_BUCKET_NAME_IMAGE, "image/", file, Constants.ALIYUN_URL_IMAGE);// 上传阿里云OSS
			return new ResultBuilder<>().data(audioUrl).build();
		} catch (Exception e) {
